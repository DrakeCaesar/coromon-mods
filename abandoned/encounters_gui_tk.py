...
def is_maximized(root):
    """True when the window is maximized - what the user calls fullscreen.

    Measured on this machine (Tk 8.6, Win11): a maximized toplevel reports "zoomed", and the
    window manager agrees - IsZoomed() is True for root.wm_frame() but False for
    root.winfo_id(), which is the inner window and never maximized. The frame handle is the
    only one that answers, so Tk's state is asked rather than reaching for ctypes.
    """
    try:
...
def maximize(root):
    """Maximize the window; Tk maps this to ShowWindow(SW_MAXIMIZE) on Windows."""
    root.state("zoomed")
...
def sanitize_geometry(text, root):
    ...
    # the minimum, and never larger than the desktop itself - an over-large size is what a
    # maximized window used to leave behind, and it cannot be undone by dragging
    w, h = min(max(w, 820), dw), min(max(h, 480), dh)
...
class GrindApp:
    def __init__(self, root, zones, species, skill_list):
        ...
        self._geom_job = None
        self.geometry = None
        self.zoomed = bool(self.state.get("zoomed", False))
        ...
        if self.zoomed:
            # after the geometry, so un-maximizing still lands on the remembered size and spot
            maximize(root)
    ...
    def save_geometry(self):
        ...
        if self.root.state() == "iconic":
            return  # minimized: geometry() reports the icon's rectangle, not the window's
        if is_maximized(self.root):
            # A maximized window's geometry() is the screen minus the taskbar. Storing that as
            # the window's size loses the real one for good - un-maximizing next run would
            # leave a window the size of the desktop. So only the fact that it is maximized is
            # remembered, and the position from before it was maximized is kept as it was.
            if not self.zoomed:
                self.zoomed = True
                self.persist()
            return
